...
@tool
def get_segment(query: str = "current") -> str:  # noqa: ARG001
...
@tool
def get_eda_summary(query: str = "dataset") -> str:  # noqa: ARG001
    ...
    Args:
        query: Ignored - always uses full dataset.
    """
...
@tool
def get_external_context(query: str = "all") -> str:  # noqa: ARG001
    """Get current external factors that might affect pricing.

    Use this when the user asks about external factors, weather impact,
    events in the area, or fuel prices.

    ⚠️ IMPORTANT: Currently returns SIMULATED data for demonstration only.

    Args:
        query: Ignored - returns all external context.
