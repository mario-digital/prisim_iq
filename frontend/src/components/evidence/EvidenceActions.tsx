...
${card.model_details.hyperparameters && Object.keys(card.model_details.hyperparameters).length > 0
  ? Object.entries(card.model_details.hyperparameters)
      .map(([key, value]) => `- **${key}:** ${value}`)
      .join('\n')
  : '*No hyperparameters documented*'}

### Input Features

${card.model_details.input_features?.length > 0
  ? card.model_details.input_features.map((f) => `- ${f}`).join('\n')
  : '*No input features documented*'}
...
**Target Users:**
${card.intended_use?.users?.length > 0
...
**Features Used:**
${card.training_data?.features_used?.length > 0
  ? card.training_data.features_used.map((f) => `- ${f}`).join('\n')
  : '*Not specified*'}

## Feature Importance

${card.feature_importance && Object.keys(card.feature_importance).length > 0
  ? `| Feature | Importance |
|---------|------------|
...
### Fairness
${card.ethical_considerations?.fairness_considerations?.length > 0
  ? card.ethical_considerations.fairness_considerations.map((c) => `- ${c}`).join('\n')
  : '*Not documented*'}

### Privacy
${card.ethical_considerations?.privacy_considerations?.length > 0
  ? card.ethical_considerations.privacy_considerations.map((c) => `- ${c}`).join('\n')
  : '*Not documented*'}

### Transparency
${card.ethical_considerations?.transparency_notes?.length > 0
...
${card.source?.preprocessing_steps?.length > 0
  ? card.source.preprocessing_steps.map((s, i) => `${i + 1}. ${s}`).join('\n')
  : '*No preprocessing steps documented*'}
...
${card.features
  .map(
    (f) =>
      `| ${f.name || 'N/A'} | ${f.dtype || 'N/A'} | ${f.description || 'N/A'} | ${f.range_or_values || 'N/A'} | ${f.distribution || 'N/A'} |`
  )
  .join('\n')}`
